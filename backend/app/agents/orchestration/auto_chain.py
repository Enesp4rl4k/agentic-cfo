...
def _conductor_allows(role: str, runnable_roles: set[str], *, has_plan: bool) -> bool:
    """When a conductor plan exists, only runnable roles may auto-chain."""
    if not has_plan:
        return True
    return role in runnable_roles
...
def _result_is_synthetic(result: dict[str, Any]) -> bool:
    """True when an agent result is kernel-extrapolated rather than measured.

    Synthetic figures (CFO financials x sector benchmarks) must not silently
    drive downstream automation — the chain is held for a human instead.
    """
    ...
    ds = (result.get("output") or result.get("posture") or {}).get("data_source")
    if ds is None:
        return False
    from app.platform.provenance import is_synthetic

    return is_synthetic(ds)
...
async def on_agent_complete(
    ...
    try:
        ...
        ctx = await get_company_context(org_id, db)
        agent_lower = agent.lower()
        upstream_synthetic = _result_is_synthetic(result)
        if upstream_synthetic:
            logger.info(
                "Auto-chain: %s result is synthetic (data_source=%s) — downstream "
                "automation held for a human for org=%s",
                agent_lower,
                (result.get("provenance") or {}).get("data_source"),
                org_id,
            )
        ...
        try:
            ...
            signals.add(f"{agent_lower}_complete")
            if upstream_synthetic:
                signals.add("synthetic_upstream")
        ...
        has_conductor_plan = conductor_plan is not None

        # ── Step 1: Run chained agents ────────────────────────────────────────
        # A synthetic (kernel-extrapolated) upstream result never auto-runs
        # downstream agents — it is surfaced for a human instead.
        downstream = [] if upstream_synthetic else AGENT_CHAIN.get(agent_lower, [])
        for next_agent in downstream:
            if not _conductor_allows(next_agent, runnable_roles, has_plan=has_conductor_plan):
                logger.debug(
        ...
        if agent_lower in CEO_TRIGGER_AGENTS and not upstream_synthetic:
            if (
                ctx.last_cfo_result is not None
                and _conductor_allows("ceo", runnable_roles, has_plan=has_conductor_plan)
            ):
                logger.info(
            ...
            elif ctx.last_cfo_result is not None and has_conductor_plan:
                logger.debug(
                    "Auto-chain: conductor skipped CEO synthesis for org=%s",
                    org_id,
                )
        ...
        for signal_fn, target_agent, label in feedback_rules:
            try:
                if signal_fn(ctx):
                    # Feedback is an inquiry: do not require conductor should_run
                    # (target often has no data yet — that is why we ask).
        ...
        # ── Step 5: Rebuild semantic company model + decision brief ───────────
        try:
            await rebuild_semantic_snapshot(org_id, db, include_brief=True)
        except Exception as exc:
            logger.debug("Semantic rebuild in auto_chain skipped: %s", exc)
...
async def _send_feedback_notification(
    ...
    try:
        ...
        await svc.create(
            ...
            domain=from_agent,
            message=message,
            source="auto_chain.feedback_loop",
            priority_score=65,
        )
        logger.info(
