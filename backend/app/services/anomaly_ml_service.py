...
def _build_feature_matrix(transactions: list[dict]) -> tuple[list[list[float]], list[str]]:
    ...
    for t, amt in zip(transactions, amounts):
...
def _run_isolation_forest(
    ...
    try:
        ...
        results = []
        for tx_id, pred, score in zip(ids, predictions, norm_scores):
            if pred == -1:  # anomaly
...
def _run_dbscan(
    ...
    try:
        ...
        results = []
        for tx_id, label in zip(ids, labels):
...
class AnomalyMLService:
    ...
    def fit_and_detect(
        ...
        if_score_map = {tx_id: score for tx_id, score in if_results}
